...
<div class="auth-wrap min-h-screen bg-gray-950 flex items-center justify-center px-4 py-10 relative overflow-hidden">
    ...
    <div class="relative z-10 w-full max-w-md">
        ...
        <div class="fade-up bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8" style="animation-delay:0.05s">
            ...
            <div class="mb-7">
                <h1 class="text-2xl font-black text-white mb-1">Set new password</h1>
                <p class="text-sm text-gray-400">Choose a strong password for your account</p>
            </div>
            <form method="POST" action="{{ route('password.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email', $request->email) }}" placeholder="you@example.com" required autofocus autocomplete="username" class="auth-input {{ $errors->get('email') ? 'border-red-500/50' : '' }}">
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-semibold text-gray-300 mb-2">Confirm New Password</label>
                    <input id="password_confirmation" type="password" name="password_confirmation" placeholder="Repeat your new password" required autocomplete="new-password" class="auth-input {{ $errors->get('password_confirmation') ? 'border-red-500/50' : '' }}">
                ...
                <div class="pt-1"><button type="submit" class="auth-btn">Reset password</button></div>
            </form>
            <p class="text-center text-sm text-gray-500 mt-6">
                Remember it now? <a href="{{ route('login') }}" class="text-white font-semibold hover:text-amber-400 transition-colors ml-1">Back to sign in →</a>
            </p>
        </div>
    </div>
</div>
